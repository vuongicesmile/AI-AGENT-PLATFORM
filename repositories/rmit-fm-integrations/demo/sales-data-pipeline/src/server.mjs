...
const server = createServer(async (request, response) => {
  ...
  if ((request.method === "GET" && url.pathname === "/api/demo")
    ...
    try {
      const fileName = url.searchParams.get("file") || "Sales_2026_08.csv";
      respondJson(response, 200, await runDemo({ fileName }));
    } catch (error) {
...
server.listen(port, "127.0.0.1", () => {
  process.stdout.write(`Sales data pipeline demo: http://127.0.0.1:${port}\n`);
});
